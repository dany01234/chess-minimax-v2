...
class Board:
    ...
    def make_move(self, piece, x, y, keep_history=False):    # history is logged when ai searches for moves
        ...
        old_y = piece.y
        if keep_history:
            if isinstance(self.board[old_x][old_y], ChessPiece):      #### fixes atr error from this line for black for some reason
    ...
    def is_valid_move(x, y):
        ...
        '''
    def has_promotion(self, color):    #new chunk #######################
        for column in range(8):
            if isinstance(self[0][column], Pawn):
                if self[0][column].color == 'black':    
                   self[0][column] = Queen(color='black')
        print('BOARD inside has_promotion for black')     

    def has_promotion(self, color):    
        for column in range(8):
            if isinstance(self[0][column], Pawn):
                if self[7][column].color == 'white':    
                   self[7][column] = Queen(color='white')
        print('BOARD inside has_promotion for white',self)   
        '''
